...
为防止欺诈，DAS 做了一些限制，即某些字符集不能同时出现在账户名当中。详细规则，请参考：[字符集](faq.md)
...
这得益于 DAS 所依赖的 Nervos CKB 公链，相关的技术细节可以参考[BTC 地址如何管理 CKB 链上的资产](faq.md)
...
DAS 内置了一些解析记录类型，如区块链地址，个人信息等。同时也支持用户和开发这自定义解析记录类型，以支持丰富的应用场景。详情请参考[解析记录命名空间约定](faq.md)。
