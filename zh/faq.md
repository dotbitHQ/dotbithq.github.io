...
设置 .bit 别名时，相关信息需要存储在区块链上，存储数据需要 201 CKB 存储空间。删除 .bit 别名时，201 CKB 将自动返还到你的余额中。修改 .bit 别名无需任何费用。
...
你的 .bit 别名会失效，但不会被清空。当 alice.bit 账户被系统回收后，这个账户的所有数据会被清空，你的 .bit 别名将失效。你可以随时删除 .bit 别名，与此同时 201 CKB 将自动返还到你的余额中。

### 如果我设置的 .bit 别名的 .bit 账户出售或转移给他人了，我的 .bit 别名会清空吗？

你的 .bit 别名会失效，但不会被清空。当账户被出售或 Owner 权限被转移时，这个账户的所有数据会被清空，你的 .bit 别名将失效。你可以随时删除 .bit 别名，与此同时 201 CKB 将自动返还到你的余额中。
...
铸造子账户费用很低，几乎为 0。为防止子账户被滥用，铸造 1 个子账户费用为 1 CKB/年。
...
### 为什么我无法编辑子账户数据？为什么我无法铸造新的子账户？

由于子账户编辑数据的交易和父账户铸造新的子账户的交易相关，因此二者无法同时进行。如果你遇到这样的情况，请等待 5 分钟后再试。
...
子账户数量没有上限。同一个父账户可以铸造出无限量的子账户，但单次铸造上限为 50 个。
...
目前，.bit 子账户仅面向 .bit 合作伙伴开放。如果你希望在你的社区或产品中使用 .bit 子账户，请发邮件到 supermancy@did.id，并在邮件正文中附上你的项目简介和简述你将如何分发 .bit 子账户。

### 子账户铸造规则是什么？

子账户注册规则与父账户一致。只能包含一种语言和数字、部分 Emoji、“-” 结合。规则详情
...
### 转换为 Ethereum 上的 .bit NFT后，如何转移给其他人？
...
是的。账户被回收后，存储押金将自动退回至该账户原 Owner 的的 [.bit 余额](https://balance.did.id/)中。你可以使用对应的钱包地址连接 .bit 余额查看。
